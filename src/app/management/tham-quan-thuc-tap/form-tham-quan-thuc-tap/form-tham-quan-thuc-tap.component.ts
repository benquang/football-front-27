...
import { IThamQuanThucTap, IThamQuanThucTapDTO } from '@models/management/tham-quan-thuc-tap.model';
...
@UntilDestroy()
@Component({
  selector: 'app-form-tham-quan-thuc-tap',
  templateUrl: './form-tham-quan-thuc-tap.component.html',
  styleUrls: ['./form-tham-quan-thuc-tap.component.scss'],
})
export class FormThamQuanThucTapComponent implements OnInit, AfterViewInit {
  @Input() modalData!: ModalData<IThamQuanThucTap>;
  ...
  form!: FormGroup<MakeForm<IThamQuanThucTapDTO>>;
  ...
  callApi() {
    if (this.modalData.action === SystemConstant.ACTION.EDIT) {
      this.thamQuanThucTapSvc.update(this.form.value, this.modalData.data.id)
        .subscribe({
          next: () => {
      ...
      this.thamQuanThucTapSvc.create(this.form.value)
        .subscribe({
          next: () => {
            this.closeModal.emit(true);
