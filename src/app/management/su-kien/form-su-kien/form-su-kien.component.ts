...
import { ISuKien, ISuKienDTO } from '@models/management/su-kien.model';
...
@UntilDestroy()
@Component({
  selector: 'app-form-su-kien',
  templateUrl: './form-su-kien.component.html',
  styleUrls: ['./form-su-kien.component.scss'],
})
export class FormSuKienComponent implements OnInit, AfterViewInit {
  @Input() modalData!: ModalData<ISuKien>;
  ...
  form!: FormGroup<MakeForm<ISuKienDTO>>;
  ...
  callApi() {
    if (this.modalData.action === SystemConstant.ACTION.EDIT) {
      this.suKienSvc.update(this.form.value, this.modalData.data.id)
        .subscribe({
          next: () => {
      ...
      this.suKienSvc.create(this.form.value)
        .subscribe({
          next: () => {
            this.closeModal.emit(true);
