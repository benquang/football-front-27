...
import { IDoanhNghiepLk, IDoanhNghiepLkDTO } from '@models/management/doanh-nghiep-lk.model';
...
@UntilDestroy()
@Component({
  selector: 'app-form-doanh-nghiep-lk',
  templateUrl: './form-doanh-nghiep-lk.component.html',
  styleUrls: ['./form-doanh-nghiep-lk.component.scss'],
})
export class FormDoanhNghiepLkComponent implements OnInit {
  @Input() thuTu = 1;
  @Input() modalData!: ModalData<IDoanhNghiepLk>;
  ...
  form!: FormGroup<MakeForm<IDoanhNghiepLkDTO>>;
  ...
  createForm(): void {
    ...
    if (this.modalData.action === SystemConstant.ACTION.EDIT) {
      this.form.patchValue({
        ...
        tenDoanhNghiep: this.modalData.data.tenDoanhNghiep,
  ...
  callApi() {
    if (this.modalData.action === SystemConstant.ACTION.EDIT) {
      this.doanhNghiepLkSvc.update(this.form.value, this.modalData.data.id)
        .subscribe({
          next: () => {
      ...
      this.doanhNghiepLkSvc.create(this.form.value)
        .subscribe({
          next: () => {
            this.closeModal.emit(true);
